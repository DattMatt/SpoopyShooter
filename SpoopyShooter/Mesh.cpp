...
Mesh::Mesh(char* model, ID3D11Device* device)
{
	// File input object
...
int Mesh::GetIndexCount() { return numIndices; }

void Mesh::CreateBuffers(Vertex* vertices, int _numVertices, UINT* indices, int _numIndices, ID3D11Device* device)
{
	CalculateTangents(vertices, _numVertices, indices, _numIndices);
	...
	vbd.ByteWidth = sizeof(Vertex) * _numVertices;       // 3 = number of vertices in the buffer
	...
	ibd.ByteWidth = sizeof(int) * _numIndices;         // 3 = number of indices in the buffer
